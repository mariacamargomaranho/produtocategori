...
def seed_database():
    with Session(motor) as sessao:
        ...
        for categoria in seed_data:
            cat = Categoria()
            print(f"Semeado a categoria {categoria['categoria']}...")
...
def seleciona_categoria():
    nome_parcial = input("Digite uma parte do nome categoria desejada: ")
    ...
    with Session(motor) as sessao:
        ...
        for categoria in rset:
            ...
            contador = contador + 1
        cod = int(input("Digite o numero de categoria desejada: "))
        categoria = ids[cod - 1]
    return categoria
...
def remover_categoria():
    ...
    with Session(motor) as sessao:
        ...
        for produto in categoria.lista_de_produtos:
            print(f"   - {produto.nome}")
        resposta = input("Remove o mesmo assim (S/N)?")
        if resposta.upper() == "S":
